...
1. We can make use of [regular expression](https://rstudio.github.io/cheatsheets/strings.pdf){target="_blank"} with
   str_detect for filtering, cleaning etc.
   ```{r}
    
   ```

## Common functions - dplyr::pivot_wider {.smaller .scrollable}
...
1. We can make use of [pivot_wider](https://tidyr.tidyverse.org/reference/pivot_wider.html){target="_blank"} to reshape our data
   from long to wide
   ```{r}
   data_wide <- data %>%
...
## Common functions - dplyr::pivot_longer
